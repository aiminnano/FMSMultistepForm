...
function SelectPlan({step, setStep, option, setOption, plan, setPlan, isOn, setIsOn}){
    ...
    return(
        <>
            <div className={`${step === 2 ? 'flex' : 'hidden'} flex-col ${isOn ? 'md:gap-[58.5px]' : 'md:gap-20' } gap-30  w-full`}>
                <div className="relative z-1 flex flex-col items-start shadow-2xl md:shadow-none rounded-xl gap-5 md:gap-9 w-full max-w-[768px] p-6 pt-8 pb-8 md:p-10 md:pl-25 md:pr-25 md:max-w-[643px] bg-white">      
                    ...
                    <div className="flex flex-col md:flex-row items-start gap-3 md:gap-4 w-full text-blue950 w-full">
                        <div className={`flex flex-start md:flex-col gap-4 md:gap-9 border border-purple200 hover:border-purple600 w-full p-4 rounded-lg ${option === 'arcade' ? 'bg-blue50 border-purple600' : 'border-purple200' } cursor-pointer`} onClick={() => setOption('arcade')}>
                            <img src={arcade} className='w-[40px] h-[40px]' alt="arcade-icon" />
                            <div className='flex flex-col items-start'>
                                <div className='text-blue950 font-medium'>Arcade</div>
                                <div className='text-sm text-grey500'>{isOn ? '$90/yr' : '$9/mo' }</div>
                                <div className={`${isOn ? 'block' : 'hidden'} text-blue950 text-[13px] pt-[2px]`}>2 months free</div>
                            </div>
                        </div>
                        <div className={`flex flex-start md:flex-col gap-4 md:gap-9 border border-purple200 hover:border-purple600 w-full p-4 rounded-lg cursor-pointer ${option === 'advanced' ? 'bg-blue50 border-purple600' : 'border-purple200'}`} onClick={() => setOption('advanced')}>
                            <img src={advanced} className='w-[40px] h-[40px]' alt="arcade-icon" />
                            <div className='flex flex-col items-start'>
                                <div className='text-blue950 font-medium'>Advanced</div>
                                <div className='text-sm text-grey500'>{isOn ? '$120/yr' : '$12/mo' }</div>
                                <div className={`${isOn ? 'block' : 'hidden'} text-blue950 text-[13px] pt-[2px]`}>2 months free</div>
                            </div>
                        </div>
                        <div className={`flex flex-start md:flex-col gap-4 md:gap-9 border border-purple200 hover:border-purple600 w-full p-4 rounded-lg cursor-pointer ${option === 'pro' ? 'bg-blue50 border-purple600' : 'border-purple200'}`} onClick={() => setOption('pro')}>
                            <img src={pro} className='w-[40px] h-[40px]' alt="arcade-icon" />
                            <div className='flex flex-col items-start'>
                                <div className='text-blue950 font-medium'>Pro</div>
                                <div className='text-sm text-grey500'>{isOn ? '$150/yr' : '$15/mo' }</div>
                                <div className={`${isOn ? 'block' : 'hidden'} text-blue950 text-[13px] pt-[2px]`}>2 months free</div>
                            </div>
                ...
                <div className="flex items-center place-content-between md:pl-25 bg-white w-full p-4 md:p-0 md:pr-25 fixed bottom-0 left-0 right-0 md:relative md:bottom-auto md:left-auto md:right-auto ">
                    ...
                    <button type="button" className=" hover:bg-blue300 bg-blue950 text-white text-sm p-3 md:pl-6 md:pr-6 rounded-md md:text-base cursor-pointer" onClick={() => setStep(3)}>Next Step</button>
                </div>
            </div>
        </>
    )
