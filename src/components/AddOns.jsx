...
function AddOns({step, setStep, option, setOption, plan, setPlan, isOn, onlineService, setOnlineService, largerStorage, setLargerStorage, customProfile, setCustomProfile, setTotalPerMonth, setTotalPerYear, setOptionValuePM, setOptionValuePY}){
    ...
    return(
        <>
            <div className={`${step === 3 ? 'flex' : 'hidden'} flex-col gap-30 md:gap-11.5 w-full`}>
                <div className="relative z-1 flex flex-col items-start shadow-2xl md:shadow-none rounded-xl gap-5 md:gap-9 w-full max-w-[768px] p-6 pt-8 pb-8 md:p-10 md:pl-25 md:pr-25 md:max-w-[643px] bg-white">
                    ...
                    <div className="flex flex-col items-start gap-4 md:gap-4 w-full text-blue950">
                        <div className={`flex items-center gap-4 place-content-between border border-purple200 w-full p-3 md:p-5.5 md:pt-4.5 md:pb-4.5 rounded-lg ${onlineService ? 'border-purple600 bg-blue50' : 'border-purple200' }`}>
                            <div className='flex items-center gap-3.5 md:gap-5.5'>   
                                <div className={`flex items-center justify-center border border-purple200 w-5.5 h-5.5 rounded-sm cursor-pointer ${onlineService ? 'bg-purple600' : 'bg-white' }`} onClick={() => setOnlineService(!onlineService)}>
                                    {onlineService && <span><img src={checkmark} alt="checked-icon" /></span> }
                                </div>
                        ...
                        <div className={`flex items-center gap-4 place-content-between border border-purple200 w-full p-3 md:p-5.5 md:pt-4.5 md:pb-4.5 rounded-lg ${largerStorage ? 'border-purple600 bg-blue50' : 'border-purple200' }`}>
                            <div className='flex items-center gap-3.5 md:gap-5.5'>
                                <div className={`flex items-center justify-center border border-purple200 w-5.5 h-5.5 rounded-sm cursor-pointer ${largerStorage ? 'bg-purple600' : 'bg-white' }`} onClick={() => setLargerStorage(!largerStorage)}>
                                    {largerStorage && <span><img src={checkmark} alt="checked-icon" /></span> }
                                </div>
                                <div className='flex flex-col leading-4.5 md:gap-1 items-start'>
                        ...
                        <div className={`flex items-center gap-4 place-content-between border border-purple200 w-full p-3 md:p-5.5 md:pt-4.5 md:pb-4.5 rounded-lg ${customProfile ? 'border-purple600 bg-blue50' : 'border-purple200' }`}>
                            <div className='flex items-center gap-3.5 md:gap-5.5'>
                                <div className={`flex items-center justify-center border border-purple200 w-5.5 h-5.5 rounded-sm cursor-pointer ${customProfile ? 'bg-purple600' : 'bg-white' }`} onClick={() => setCustomProfile(!customProfile)}>
                                    {customProfile && <span><img src={checkmark} alt="checked-icon" /></span> }
                                </div>
                                <div className='flex flex-col leading-4.5 md:gap-1 items-start'>
